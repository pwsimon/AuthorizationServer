...
HRESULT CoAuthServiceCall::Dispose()
{
	if (NULL != m_pRenewTokenAsync)
	{
		// es war EIN Renew noetig, resource jetzt freigeben
		m_pRenewTokenAsync->m_spRequest->put_onreadystatechange(NULL);
		m_pRenewTokenAsync->ExternalRelease();
	}
	return m_spRequest->put_onreadystatechange(NULL);
}
